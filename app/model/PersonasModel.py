...
class Personas:
    ...
    def select(self, correo):
        try:
            ...
            cursor = conexion.cursor()
            sql = "SELECT correo, contraseña, tipopersona, idrutina, iddieta, idtest, documento, nombres, imagen FROM personas where correo = %s"
    ...
    def estadisticas(self):
        try:
            ...
            sql = "SELECT count(*) FROM personas WHERE tipopersona = 'admin'"
            cursor.execute(sql,)
            diccionario = cursor.fetchall()
            admin = diccionario[0]
            conexion.commit()
